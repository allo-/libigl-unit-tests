...
class delaunay_triangulation : public ::testing::TestWithParam<std::string> {};
...
TEST(delaunay_triangulation, git_issue_521) {
    ...
    ASSERT_LT(0, F.rows());
    ASSERT_EQ(3, F.cols());
    ASSERT_GT(16, F.maxCoeff());
    ASSERT_EQ(0, F.minCoeff());
}
